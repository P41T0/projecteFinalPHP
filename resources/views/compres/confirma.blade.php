<x-guest-layout>
    ...
    @switch($missatge)
        @case(0)
        <p>{{__("La comanda ja s'ha tancat anteriorment, el preu total de la comanda és de")}} {{$preuTotal}} € {{__("i la comanda es passarà a recollir a la botiga de ")}}{{$botiga}}</p>
            @break
        @case(1)
        <p>{{__("No hi ha cap producte en la comanda seleccionada")}}</p>
            @break
        @case(2)
        <p>{{__("Comanda tancada, el cost total és de")}}{{$preuTotal}}€ {{__("i la comanda es passarà a recollir a la botiga de ")}}{{$botiga}}</p>
        @break
        @case(3)
